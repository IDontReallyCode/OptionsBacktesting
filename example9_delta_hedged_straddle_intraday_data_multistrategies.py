...
def main():
    ...
    """
    myaccount1 = obt.Account(deposit=2000)
    myaccount2 = obt.Account(deposit=2000)
    myaccounts = [myaccount1, myaccount2]
    ...
    mystrategy1 = MyStrategy(ratioRV_tohedge= 1, ratioPNL_takeprofit= .5, ratioPNL_takeloss= -0.5, ratioRV_takeloss= 0.5)
    mystrategy2 = MyStrategy(ratioRV_tohedge= 2.5, ratioPNL_takeprofit= .75, ratioPNL_takeloss= -0.75, ratioRV_takeloss= 0.75)
    mystrategies = [mystrategy1, mystrategy2]
    
    mychronos = obt.Chronos(marketdata=mymarket, marketdealer=mydealer, clientaccounts=myaccounts, clientstrategies=mystrategies, chronology=uniquetimesteps)
    ...
    ax1.plot(uniquetimesteps[-len(myaccount1.totalvaluests):], myaccount1.totalvaluests)
    ax1.yaxis.set_major_formatter('${x:1.2f}')
    ax1.set_title('Account value')
    ax2.scatter(uniquetimesteps[-len(myaccount1.totalvaluests):],stockdata.iloc[-len(myaccount1.totalvaluests):]['close'],s=1)
    ax2.yaxis.set_major_formatter('${x:1.2f}')
    ax2.legend()
    ax2.set_title('Stock Price')
    ax3.plot(uniquetimesteps[-len(mystrategy1.stockpositions):], mystrategy1.stockpositions)
    ...
    fig, (ax1,ax2,ax3) = plt.subplots(3,1, sharex=False, sharey=False)
    ax1.plot(uniquetimesteps[-len(myaccount2.totalvaluests):], myaccount2.totalvaluests)
    ax1.yaxis.set_major_formatter('${x:1.2f}')
    ax1.set_title('Account value')
    ax2.scatter(uniquetimesteps[-len(myaccount2.totalvaluests):],stockdata.iloc[-len(myaccount2.totalvaluests):]['close'],s=1)
    ax2.yaxis.set_major_formatter('${x:1.2f}')
    ax2.legend()
    ax2.set_title('Stock Price')
    ax3.plot(uniquetimesteps[-len(mystrategy2.stockpositions):], mystrategy2.stockpositions)
    ax3.set_title('Stock positions')
    # ax4.plot(uniquetimesteps[-len(mystrategy.deltavalues):], mystrategy.deltavalues)
    # ax4.set_title('Delta values')
    # ax5.stem(mystrategy.buydates,1*np.ones(len(mystrategy.buydates),),linefmt='green')
