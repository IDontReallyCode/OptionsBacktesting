...
class MyStrategy(obt.abstractstrategy.Strategy):
    ...
    def estimatestrategy(self, marketfeedback, accountfeedback):
        ...
        tickercandle = self.marketdata.tickerlist[0].getcurrentstockcandle()
        ...
        if self.marketdata.currentdatetime.weekday()>=4: #Friday after close => we submit order to open the position
            # Friday has passed, we put an order in for next Monday, SHORT a put of 21+ dte

            # This is where we need to be able to assess the margin requirement of a potential trade
            shortputorder = obt.Order(tickerindex = 0, ticker=targetput.iloc[0]['ticker'], assettype=obt.ASSET_TYPE_OPTION, symbol= targetput.iloc[0]['symbol'], 
                                    action=obt.SELL_TO_OPEN, quantity=+1, tickerprice=tickercandle.iloc[0]['close'], optionprice=targetput.iloc[0]['ask'], ordertype=obt.ORDER_TYPE_MARKET, pcflag=targetput.iloc[0]['pcflag'], 
                                    k=targetput.iloc[0]['k'], expirationdate=targetput.iloc[0]['expirationdate'])

            self.theseorders.append(shortputorder)
            self.theseorders.append(obt.Order(tickerindex = 0, ticker=targetput.iloc[0]['ticker'], assettype=obt.ASSET_TYPE_STOCK, symbol=targetput.iloc[0]['ticker'], 
                                    action=obt.SELL_TO_OPEN, quantity=20))
            ...
            if optionpositions:
                ...
                if newdelta<-1:
                    ...
                    if quantityadjustementtodeltahedge>0:
                        ...
                        self.theseorders.append(obt.Order(tickerindex = 0, ticker=targetput.iloc[0]['ticker'], assettype=obt.ASSET_TYPE_STOCK, symbol=targetput.iloc[0]['ticker'], 
                                action=obt.SELL_TO_CLOSE, quantity=quantityadjustementtodeltahedge))
            ...
            if optionpositions:
                optionsymbol = list(optionpositions.keys())[0]
                optionstobuyback = self.account.positions.getoptionquantity(targetput.iloc[0]['ticker'], optionsymbol)
...
def main():
    myaccount = obt.Account(deposit=10000, margintype=obt.MARGINTYPE_NONE)
